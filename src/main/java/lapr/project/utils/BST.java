...
public class BST<E extends Comparable<E>> implements lapr.project.utils.BSTInterface<E> {
    ...
    public void insert(E element) {
        if (root == null) {
            Node root = new Node(element, null, null);
            this.root = root;
        } else {
    ...
    public Map<Integer, List<E>> nodesByLevel() {
        int level = 0;
        Map<Integer, List<E>> nodesInLevel = new HashMap<>();
        if (root != null)
            processBstByLevel(root, nodesInLevel, level);
        return nodesInLevel;
    }
